...
public:
	...
	bool check_for_wins(unsigned board_index) {
		...
		for (unsigned z = 0; z < 5; z++) {
			if ((board[z][0].second && board[z][1].second && board[z][2].second &&
				 board[z][3].second && board[z][4].second) ||
				(board[0][z].second && board[1][z].second && board[2][z].second &&
				 board[3][z].second && board[4][z].second)) {
				return true;
			}
		}
